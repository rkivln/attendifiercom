...
import { ClipboardCheck, Users, Clock, Download, ArrowRight, Zap } from "lucide-react";
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <nav className="flex items-center justify-between px-8 py-5 border-b border-border/50">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-lg font-semibold text-foreground tracking-tight">Attendifier</span>
        </div>
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-20">
        <div className="text-center max-w-2xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full frosted-surface text-primary text-sm font-medium mb-8">
            <Zap className="h-3.5 w-3.5" />
            College Attendance System
          </div>
          <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-5 leading-[0.95] tracking-[-0.04em]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-20">
            {[
              ...
              <div key={f.title} className="content-card text-left hover:border-primary/20 transition-colors">
                <div className="h-10 w-10 rounded-lg frosted-surface flex items-center justify-center mb-3">
                  <f.icon className="h-5 w-5 text-primary" />
                </div>
                <h3 className="font-semibold text-foreground text-sm tracking-tight">{f.title}</h3>
